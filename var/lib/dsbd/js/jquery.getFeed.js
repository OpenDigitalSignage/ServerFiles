...
JRss.prototype = {
  _parse: function (xml) {
    ...
    this.title = jQuery(channel).find('title:first').html();
    this.description = jQuery(channel).find('description:first').html();
    ...
    jQuery('item', xml).each(function () {
      var item = new JFeedItem();
      item._element = this;
      item.title = jQuery(this).find('title').eq(0).html();
      item.description = jQuery(this).find("encoded").eq(0).html();
      if (item.description === undefined) {
        item.description = jQuery(this).find('description').eq(0).html();
      }
